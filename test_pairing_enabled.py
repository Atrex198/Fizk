"""
Test script to verify pairing verification is properly enabled
"""
import numpy as np
import torch
...
def test_pairing_verification():
    ...
    print(f"✅ Circuit generated: {circuit.num_constraints} constraints")
    ...
    if result.is_valid:
        print("✅ Test 1: Valid proof accepted")
        tests_passed += 1
    else:
        print("❌ Test 1: Valid proof REJECTED (should pass)")
    
    if not result_tampered.is_valid:
        print("✅ Test 2: Tampered proof rejected")
        tests_passed += 1
    else:
        print("❌ Test 2: Tampered proof ACCEPTED (should fail)")
    ...
    if result.details and 'pairing_checks' in result.details:
        ...
        if pairing_status == 'enabled':
            print("✅ PAIRING VERIFICATION IS ENABLED!")
        else:
            print("❌ PAIRING VERIFICATION IS NOT ENABLED!")
